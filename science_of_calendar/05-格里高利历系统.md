...
$$
\Delta T(y) = (y + 45) \times (365.2500 - 365.242190402) = (y + 45) \times 0.007809598 \text{ 日}
$$

到1582年，经过约1627年的累积：

$$
\Delta T(1582) = (1582 + 45) \times 0.007809598 \approx 1627 \times 0.00781 \approx 12.7 \text{ 日}
$$

然而历史记录显示当时的偏差约为10日。这个差异源于两个因素：
...
$$
\text{累积误差}(y) = \int_{-45}^{y} \left(365.25 - T_{\text{tropical}}(y')\right) dy'
$$

其中 $T_{\text{tropical}}(y')$ 是真实回归年长度，随时间缓慢变化。
...
|------|----------|---------------------|-------------------|
| 儒略历 | 365.250000日 | +0.007809598日/年 | ~128年 |
...
### 5.3.3 处理1582年大跳跃：重构"丢失的十天"的代码逻辑

在所有排历软件中，最容易出 Bug 的就是 1582 年 10 月的那个"黑洞"。
1582年10月4日（星期四）的下一天，直接变成了1582年10月15日（星期五）。那么，当我们说起"1582年10月10日"时，它是哪一天？

为了处理这一历史创举，现代软件（如 Java 的 `java.util.GregorianCalendar` 或 Python 的某些天文库）通常采用**切变点架构（Cutover Architecture）**。

在程序引擎内部，系统并非分别存储儒略历和格里高利历的计算函数，而是维护一个统一的绝对线性时间轴（通常使用儒略日 JDN）。当需要将绝对天数转为人类可读的年月日时，引擎会执行一个被称为 **格里高利历切变（Gregorian Cutover）** 的判定：

```python
# 1582年10月15日的儒略日是 2299161
GREGORIAN_CUTOVER_JDN = 2299161

def absolute_day_to_ymd(jdn: int) -> tuple[int, int, int]:
    """工业级：带切变点的绝对日数转公历/儒略历函数"""
    if jdn >= GREGORIAN_CUTOVER_JDN:
        # 使用 Fliegel-Van Flandern 纯整数格里历算法
        return jd_to_gregorian_integer(jdn)
    else:
        # 使用 Fliegel-Van Flandern 纯整数儒略历算法
        return jd_to_julian_integer(jdn)

def ymd_to_absolute_day(year: int, month: int, day: int) -> int:
    """带切变点的年月日转绝对日数"""
    if year > 1582 or (year == 1582 and month > 10) or (year == 1582 and month == 10 and day >= 15):
        jdn = gregorian_to_jd_integer(year, month, day)
    elif year < 1582 or (year == 1582 and month < 10) or (year == 1582 and month == 10 and day <= 4):
        jdn = julian_to_jd_integer(year, month, day)
    else:
        raise ValueError("日期落在1582年的历史黑洞中！不存在此日。")
    return jdn
```

在这套切变架构下计算日期差变得异常简单。例如，我们要计算拿破仑时代的某天与凯撒时代的某天相隔多少日：
系统会把它们都转换为统一的 JDN 整数，直接执行减法即可。完全无需去纠结期间跳过了多少个十天、双闰日或是复杂的国家演变。

**Proleptic Gregorian Calendar（前格里高利历）**

值得一提的是，在天文学（如 NASA/JPL ISO 8601 标准）或某些金融软件中，为了避免上述 IF/ELSE 切变带来的逻辑分支性能损耗，人们发明了**前格里高利历（Proleptic Gregorian Calendar）**。
所谓前公历，就是**假设在1582年之前，格里高利历也一直存在**。
在这种历法下：
- 所有历法数学退化为完美的纯粹周期函数。
- 但代价是它与实际的历史记载（Julius Caesar 出生的具体日期）存在几天的偏移。

对于需要查询历史记录的人类，使用带 Cutover 的混合历（Mixed Calendar）；对于进行轨道推算的高性能计算机，永远使用前格里高利历。这是历法科学在软件工程中做出的最伟大的解耦设计。
