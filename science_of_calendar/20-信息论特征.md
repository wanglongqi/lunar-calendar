...
- $Y$ 是输出（历法日期）
- $H(Y)$ 是输出的熵
- $H(Y\vert X)$ 是条件熵（给定输入时输出的不确定性）

**完美历法**的理想容量：

如果历法能够完美编码天文周期（无误差），则 $H(Y\vert X) = 0$ ，信道容量等于输出的熵：
...
如果历法存在误差（如儒略历的季节漂移），则 $H(Y\vert X) > 0$ ，信道容量降低：
...
def is_leap_gregorian(year):
    if year % 4 != 0:        # 边1, 分支1
        return False
    elif year % 100 != 0:     # 边2, 分支2
        return True
    elif year % 400 == 0:     # 边3, 分支3
        return True
    else:                     # 边4, 分支4
        return False
```

控制流图（Control Flow Graph, CFG）分析：
- 节点数（Nodes, N）：判定节点（if/elif） + 返回节点 = 8
- 边数（Edges, E）：连接各判定和返回的路径 = 10
- 连通分量（Connected Components, P）：1（单函数）
...
这四个独立路径完美对应了格里高利历的四种年份类型划分逻辑（非闰年，普通闰年，特殊平年，世纪闰年）。
...
其中：
- $H(Y)$ ：年份的熵（取决于年份类型分布）
- $H(M\vert Y)$ ：给定年份的月份熵（取决于闰月规则）
- $H(D\vert Y, M)$ ：给定年和月的日熵（取决于闰年规则）
...
2. 给定年份的月份熵：月份长度分为4种（28、29、30、31日），取决于是否为闰年：
   ...
   - 平均熵： $H(M\vert Y) \approx 1.78$ bits（见20.2.1节）
3. 给定年和月的日熵：大多数月份为30或31日，因此日的不确定性很小：
   - $H(D\vert Y, M) \approx \log_2 31 \approx 4.95$ bits（最多31日）
...
- **经典比特**：只能表示0或1
- **量子比特**：可以表示0、1或两者的叠加（Superposition， $\alpha\vert 0\rangle + \beta\vert 1\rangle$ ）
