...
1957年，印度政府正式采用以Saka纪元为基础的国家历法（Indian National Calendar），这一举措体现了独立印度对本土文化遗产的重视。印度国家历法与格里高利历并行使用，民间则主要使用格里高利历，但宗教活动和文化场合仍保留传统历法。
...
印度国家历法与格里高利历的转换需要特别注意新年开始时间的差异。格里高利历以1月1日为新年，而Saka历法的新年在3月底。因此，同一格里高利历年可能对应两个Saka历年的一部分，反之亦然。

转换算法的逻辑是：首先确定该Saka历年与格里高历历年的对应区间。通常情况下，Saka历年的第1月Chaitra 1日对应格里高利历的3月22日（平年）或3月21日（闰年）。因此，Saka历年的第1个月至第8个月通常在格里高利历的下一年。这种年份对应关系在跨年处理时需要格外小心。
...
孟加拉太阳历（Bengali calendar）是印度东北部班加罗地区特有的历法系统，其起源可追溯至7世纪统治者Sashanka时期（约公元593年）。该历法虽然基于太阳运行原理，但在月份命名、闰日位置和新年日期方面都体现了独特的地方特色。

孟加拉历的新年Pohela Boishakh是孟加拉地区最重要的节日之一，每年在格里高利历4月14日或15日庆祝。这一日期接近春分，象征着新的农业循环和文化传统。Pohela Boishakh不仅是历法转换的节点，更是文化庆祝的重要场合，人们在这一天通过特定的仪式和活动迎接新年的到来。

### 8.4.2 月份结构与闰年处理

孟加拉历的月份安排与印度国家历法有所不同。从Boishakh（第1月）到Bhadra（第6月）各有31日，Ashwin到Poush（第7-10月）各有30日，Falgun（第11月）在平年有30日、闰年有31日，Chaitra（第12月）通常有30日但可能根据闰年规则增加到31日。这种分布反映了班加罗地区对传统印度太阳历的地方化调整。

孟加拉历的闰年处理较为复杂。虽然基础规则与格里高利历相似（每四年插入一个闰日），但闰日的插入位置需要在Falgun和Chaitra月之间调整，以保证新年日期稳定在4月14日或15日。这种处理要求更加复杂的计算逻辑，也体现了地方历法在应对天文精确性与文化传统需求之间的权衡。
...
def calculate_nirayana_solar_longitude(t):
    """
    计算印度历法中的恒星太阳黄经 (Nirayana Surya Sphuta)。
    与西方占星的回归黄经 (Sayana) 不同，印度历法使用的是扣除岁差 (Ayanamsha) 后的恒星黄经。
    
    Parameters:
        t: Skyfield Time 对象
        
    Returns:
        float: 恒星太阳黄经（度，0-360°）
    """
    eph = load('de421.bsp')
    earth = eph['earth']
    sun = eph['sun']
    
    # 1. 计算视太阳在回归黄道坐标系 (Sayana) 下的黄经
    astrometric = earth.at(t).observe(sun)
    apparent = astrometric.apparent()
    _, lon, _ = apparent.ecliptic_latlon()
    sayana_lon_deg = lon.degrees
    
    # 2. 计算拉希里岁差 (Lahiri Ayanamsha)
    # 拉希里岁差的公认标准是：以公元285年左右的春分点为零点。
    # 一个常用的高精度近似公式（基于 IAU 2006 进动理论推导的近似插值）：
    # Ayanamsha(JD) ≈ 23.85 + (JD - 2451545.0) / 365.24219 * (50.290966 / 3600.0)
    # 这里的 23.85° 是 2000年初的拉希里岁差近似值
    jd = t.tt
    ayanamsha_deg = 23.85 + (jd - 2451545.0) / 365.24219 * (50.290966 / 3600.0)
    
    # 3. 恒星黄经 = 回归黄经 - 岁差
    nirayana_lon_deg = (sayana_lon_deg - ayanamsha_deg) % 360.0
    
    return nirayana_lon_deg

def rashi_index_nirayana(longitude_deg: float) -> int:
    """
    根据恒星黄经确定印度黄道星座 (Rashi) 索引
    """
    # 0-30度: Mesha, 30-60度: Vrishabha, 等
    return int(longitude_deg // 30) + 1

def rashi_name(index: int) -> str:
    rashi_names = [
        "Mesha (白羊)", "Vrishabha (金牛)", "Mithuna (双子)", "Karka (巨蟹)",
        "Simha (狮子)", "Kanya (处女)", "Tula (天秤)", "Vrishchika (天蝎)",
        "Dhanu (射手)", "Makara (摩羯)", "Kumbha (水瓶)", "Mina (双鱼)"
    ]
    return rashi_names[index - 1] if 1 <= index <= 12 else "Unknown"

if __name__ == '__main__':
    ts = load.timescale()
    # 测试日期：2024年4月14日（传统的太阳历新年 Puthandu / Vaisakhi 前后）
    t_test = ts.utc(2024, 4, 14, 12, 0)
    
    nirayana_lon = calculate_nirayana_solar_longitude(t_test)
    rashi_idx = rashi_index_nirayana(nirayana_lon)
    
    print(f"当前时间 (UTC)：{t_test.utc_strftime()}")
    print(f"恒星太阳黄经 (Nirayana)：{nirayana_lon:.4f}°")
    print(f"黄道星座 (Rashi)：{rashi_idx}. {rashi_name(rashi_idx)}")
    print("注意：如果经度刚好跨过 0°，则标志着太阳进入 Mesha（白羊宫），即印度传统历法的新年。")
```
...
孟加拉新年Pohela Boishakh和泰米尔新年Puthandu都是重要的文化节日。这些节日不仅是历法转换的标记，更是文化认同和社区凝聚力的体现。在全球化时代，地方历法和传统节庆成为文化传承的重要载体。
...
孟加拉太阳历作为地方化的太阳历系统展示了印度不同地区的文化多样性。孟加拉历起源于7世纪，其新年Pohela Boishakh是孟加拉地区重要的文化节日。泰米尔太阳历有着长达数千年的传统，其新年Puthandu反映了南印度天文传统的独立性。
