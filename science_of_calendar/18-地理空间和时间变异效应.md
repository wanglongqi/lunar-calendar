...
$$
d = \int_{\lambda_1}^{\lambda_2} \sqrt{\frac{(a^2 \cos^2 \phi + b^2 \sin^2 \phi) + (a' \cos \phi \sin \phi)^2}{a^2 \cos^2 \phi + b^2 \sin^2 \phi}} \, d\lambda
$$
...
def calculate_accurate_sun_altitude_and_refraction():
    """
    计算考虑地球大气折射效应（Atmospheric Refraction）的真实视界太阳高度角
    展示天文高度角与真实大气折射修正后的高度角差异。
    """
    ts = load.timescale()
    eph = load('de421.bsp')
    sun, earth = eph['sun'], eph['earth']
    
    # 定义观测地点（例如：北京的天安门广场周边）

    beijing = earth + wgs84.latlon(39.9, 116.4)
    
    # 设定测试时间为 2024 年夏至的正午前后

    t = ts.utc(2024, 6, 21, 4, 30) # UTC 4:30 约为北京时间 12:30 (正午)
    
    # 计算无大气折射的几何高度角

    astrometric = beijing.at(t).observe(sun)
    alt, az, distance = astrometric.apparent().altaz()
    geometric_alt = alt.degrees
    
    # 应用标准大气层折射修正 (10°C, 1010mbar 典型地面条件)

    # Skyfield 默认通过 apparent().altaz() 可以通过传入 temperature_C 和 pressure_mbar 获取

    alt_refracted, az_refracted, _ = astrometric.apparent().altaz(temperature_C=10.0, pressure_mbar=1010.0)
    refracted_alt = alt_refracted.degrees
    
    print("\n[高精度视太阳高度角及大气折射效应分析]")
    print(f"观测地：北纬 39.9°, 东经 116.4°")
    print(f"时间 (UTC)：{t.utc_strftime()}")
    print(f"几何太阳高度角：{geometric_alt:.4f}°")
    print(f"视界折射高度角：{refracted_alt:.4f}°")
    print(f"大气折射修正量：{(refracted_alt - geometric_alt) * 3600:.2f} 角秒")
    
    # --- 寻根计算日出日落，演示大气折射如何影响白昼长度 ---

    t0 = ts.utc(2024, 6, 21, 0, 0)
    t1 = ts.utc(2024, 6, 22, 0, 0)
    
    f_sunrise_sunset = almanac.sunrise_sunset(eph, wgs84.latlon(39.9, 116.4))
    t_events, y_events = almanac.find_discrete(t0, t1, f_sunrise_sunset)
    
    print("\n当天日出/日落时间 (UTC，已自动应用标准折射率和视半径 -0.8333°):")
    for t_event, event_type in zip(t_events, y_events):
        event_name = '日出 (Sunrise)' if event_type else '日落 (Sunset)'
        print(f"  {event_name}: {t_event.utc_strftime()}")

# 运行真实模型分析

calculate_accurate_sun_altitude_and_refraction()
