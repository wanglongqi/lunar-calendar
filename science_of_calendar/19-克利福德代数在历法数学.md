...
$$
R_{\text{公转}}(t) = e^{-\frac{2\pi t}{365.25} \frac{e_1 \wedge e_2}{2}}
$$
...
def sta_gps_relativistic_effects(v_sat: float = 3874.0, altitude: float = 20180000.0) -> dict:
    """
    使用时空代数 (Spacetime Algebra, STA) 计算 GPS 卫星的相对论时间膨胀。
    在 STA 中，四维闵可夫斯基时空的洛伦兹变换可以用时空双向量 (Spacetime Bivector) 构成的旋转器表示。
    
    R = exp(-θ/2 * (γ0 γ1))
    
    Parameters:
        v_sat: 卫星轨道速度 (m/s)
        altitude: 卫星距离地面的高度 (m)
    """
    # 物理常数
    c = 299792458.0  # 光速 (m/s)
    G = 6.67430e-11  # 万有引力常数
    M_E = 5.9722e24  # 地球质量 (kg)
    R_E = 6371000.0  # 地球平均半径 (m)
    
    # 时空代数中的快度 (Rapidity) θ
    beta = v_sat / c
    theta = np.arctanh(beta)
    
    # 1. 狭义相对论旋转器效应（纯运动学时间膨胀）
    # 在 STA 中，时间基底的投影 t' = t * cosh(theta)
    gamma = np.cosh(theta)
    # 每日时钟变慢量 (秒) -> 洛伦兹因子导致的延缓 (-7.2 微秒/天)
    kinematic_sr_shift = (1.0 / gamma - 1.0) * 86400.0 
    
    # 2. 广义相对论时空曲率效应（引力红移）
    # 根据施瓦西度规在 STA 的等效推导，时空双向量的尺度因子发生改变
    r_sat = R_E + altitude
    phi_sat = -G * M_E / r_sat
    phi_earth = -G * M_E / R_E
    
    delta_phi = phi_sat - phi_earth
    # 每日时钟变快量 (秒) -> 引力场较弱导致的加速 (+45.9 微秒/天)
    gravitational_gr_shift = (delta_phi / (c**2)) * 86400.0
    
    total_shift = kinematic_sr_shift + gravitational_gr_shift
    
    return {
        "kinematic_shift_us_per_day": kinematic_sr_shift * 1e6,
        "gravitational_shift_us_per_day": gravitational_gr_shift * 1e6,
        "total_net_shift_us_per_day": total_shift * 1e6,
        "distance_error_m_per_day": total_shift * c
    }

# 运行 STA 相对论推演
print("GPS 时空代数 (STA) 效应解析:")
gps_sta_result = sta_gps_relativistic_effects()
print("-" * 50)
print(f"狭义相对论时间膨胀 (STA 快度投影): {gps_sta_result['kinematic_shift_us_per_day']:.2f} 微秒/天")
print(f"广义相对论引力红移 (STA 尺度变换): +{gps_sta_result['gravitational_shift_us_per_day']:.2f} 微秒/天")
print(f"净时标漂移: +{gps_sta_result['total_net_shift_us_per_day']:.2f} 微秒/天")
print(f"如果不作相对论补偿，每天的定位漂移将达到: {gps_sta_result['distance_error_m_per_day']/1000:.2f} 公里！")
```
...
几何代数基础部分介绍了克利福德代数的起源（威廉·金顿·克利福德，1878年）和核心概念。几何积统一了传统代数中的内积（点积）和外积（楔积），能够同时表示标量（0-向量）、向量（1-向量）、双向量（2-向量）和多向量（k-向量）。三维几何代数（G3）提供了直观的旋转器和旋转表示，绕z轴旋转 $\theta$ 角的旋转器 $R_z(\theta) = e^{-\frac{\theta}{2} e_1 \wedge e_2} = \cos \frac{\theta}{2} - (e_1 \wedge e_2) \sin \frac{\theta}{2}$ 包含了明确的旋转平面和角度信息。

历法中的旋转表示部分应用了几何代数来描述地球的自转和公转。地球自转（每日周期）可以用旋转器 $R_{\text{自转}}(t) = e^{-\frac{2\pi t}{24} \frac{e_1 \wedge e_2}{2}}$ 表示，其中 $t$ 是时间， $e_1 \wedge e_2$ 是赤道平面。地球公转（年周期）可以用旋转器 $R_{\text{公转}}(t) = e^{-\frac{2\pi t}{365.25} \frac{B}{2}}$ 表示，其中 $B$ 是黄道平面的旋转平面。默冬章（Metonic cycle，19年7闰）的几何分析展示了太阳年（365.2422日）和朔望月（29.530588日）在19年周期中几乎完美同步：19个回归年约6939.6日，235个朔望月约6939.7日，差异仅约0.0866日（2.08小时）。
...
可视化与计算优势部分强调了几何代数的几何直观性和计算效率。相比传统矩阵和四元数方法，几何代数提供了更直观的几何意义：旋转器明确表示旋转平面（如 $e_1 \wedge e_2$ 表示 $x$-$y$ 平面）和旋转角度，而非抽象的虚数单位或复杂的矩阵元素。在计算效率方面，矩阵乘法的计算成本为 $O(n^3)$ ，而旋转器的乘法是常数时间 $O(k)$ （ $k$ 是双向量维度），在高维空间中优势显著。几何代数还避免了万向锁（Gimbal Lock）问题，通过明确旋转平面保持独立的旋转自由度。

数学框架对比部分系统比较了几何代数、四元数和传统矩阵/向量方法。四元数是几何代数在三维空间中的特例，四元数的 $i, j, k$ 与几何代数中的双向量 $e_2 \wedge e_3, e_3 \wedge e_1, e_1 \wedge e_2$ 对应。几何代数相比传统方法的主要优势包括：可扩展到任意维度、统一的几何意义、避免万向锁、更自然的组合性。中国干支周期（60年）和玛雅历法轮（52年）的几何解释展示了周期性历法可以用正交旋转的组合表示，揭示了历法周期性的数学本质：LCM(10, 12) = 60（干支）和LCM(260, 365) / 5 = 18,980日（玛雅）。
