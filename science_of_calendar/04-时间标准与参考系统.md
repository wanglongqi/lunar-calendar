...
### 4.4.6 民用历法边界与UTC时间追踪效应

由于UTC通过闰秒机制严格控制与UT1的偏差（ $|\Delta UT1| < 0.9$ 秒），这种约束在民用历法计算（尤其是需要处理公元前历史日期的历法，如中国古代农历推算）中有着巨大价值。

在纯天文推算中，历史时间的流逝必须通过地球力学时（TT）减去 $\Delta T$ （地球自转不均匀量）来获得真实的平太阳时（UT1）。然而 $\Delta T$ 在前几千年可高达数小时，使得绝对时间换算极为复杂。

但正是由于现代UTC定义下的时区系统等同于平移后的UT1，所以在分配某一天文事件（如朔月或冬至点）发生的**日历日期**（Civil Date）时，我们可以安全地忽略 $\Delta T$ 的日内微小浮动。以北京时间（UTC+8）为例：
因为 $\text{UT1} \approx \text{UTC}$ ，所以我们只需要对UT1的儒略日按时区偏移（ $+ \frac{8}{24}$ ）并向下取整，即可精准捕获该天文事件在历史上当日历上对应的民用日期起算点：

$$
\text{Civil\_Start\_JD} = \lfloor \text{JD}_{\text{UT1}} + \frac{8}{24} + 0.5 \rfloor - 0.5
$$

这种纯整数阶梯的"边界裁剪"，使得我们在历法排月中只需追踪 UTC 标尺即可锁定日期，巧妙地跨越了用 $\Delta T$ 反复对齐历史历法日的陷阱。
...
3. **时间跳跃规则**：
   ...
   这意味着 $\vert T_{\text{before}} - T_{\text{after}}\vert = 1$ 秒，但实际上时间"跳跃"了1秒
